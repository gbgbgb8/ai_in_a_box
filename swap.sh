...
echo "=== Final Production AI-in-a-Box Fix Script ==="
echo "Fixes all dependency issues, upgrades to Phi-3 Mini, and ensures working TTS"
echo "Version: 6.0 (Production Ready - Fully Working)"
echo ""
...
echo "Step 5: Downloading Phi-3 model..."
cd downloaded
...
echo "Step 8: Updating llm_speaker.py with Phi-3 configuration..."
cat > llm_speaker.py << 'EOF'
#!/usr/bin/env python3
"""
LLM Speaker class with Phi-3 Mini support
"""
import os
import time
...
class LLMSpeaker(object):
    def __init__(self, model_str="phi3-mini-4k-q4"):
    ...
    def llm_producer(self, prompt_str):
        ...
        for tok in resp_gen:
            ...
            if (self.model_str == "phi3-mini-4k-q4" and 
...
echo "Step 9: Updating main.py and run_chatty.sh to use Phi-3..."
sed -i 's/sys.exit(main(sys.argv\[1\]))/sys.exit(main("phi3-mini-4k-q4"))/' main.py
sed -i 's/orca3b-4bit/phi3-mini-4k-q4/' run_chatty.sh
...
echo ""
echo "The system will automatically start the AI-in-a-box interface on boot!"
echo "=========================================================="